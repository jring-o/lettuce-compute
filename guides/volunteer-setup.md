...
`scheduling.idle_threshold_mins` minutes. That needs a way to read the idle time:
Windows and macOS have one built in; on Linux the volunteer asks the desktop
session (the D-Bus ScreenSaver interface), then `xprintidle` (X11), so a headless
machine or a daemon started over SSH or as a service has none. When the idle time
cannot be read, the volunteer never starts work and says so: a WARN in the log, a
notice in the app, `Paused: idle_unknown` in `status`, and a failure or warning on
the `idle time` row of `doctor`. For finer control than whole-hour
