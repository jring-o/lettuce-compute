...
// The daemon's one-connection-per-head guarantee (the v0.4.0 double-connection
// symptom) is now provided by config.Load's entry migration, which merges
// legacy duplicate entries WITH their leaf pins preserved instead of collapsing
// them at startup and discarding the pin (PB-16). See
// config.TestLoad_MergesLegacyDuplicateServerEntries.
