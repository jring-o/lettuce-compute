...
// request context by the server router AFTER authentication, so the audit admin
// endpoints can enforce operator-only access WITHOUT importing the server package
// (import cycle). This mirrors trust.Caller / standing.Caller / credit.Caller exactly:
// the router's authAdmin wrapper must inject this alongside those — a handler wrapped
// in authAdmin WITHOUT the matching in-handler admin check is fail-open, and a handler
...
// upstream) yields the zero value — IsAdmin=false — so the audit admin API fails
// closed.
