...
// SeenWithin performs an atomic SET NX PX. On a store error it returns
// (false, err) so the caller's fail-open / fail-closed policy decides; it never
// fabricates an alreadySeen=true on error (that would silently reject legitimate
