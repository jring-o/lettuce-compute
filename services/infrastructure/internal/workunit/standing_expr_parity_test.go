...
// TestStandingExprSQL_MatchesEffectiveStanding pins the two twins of the effective account
// standing (BG-24b) against each other: the Go source of truth
// volunteer.EffectiveStanding, which result stamping and validation countability use, and
// standingExprSQL, the SQL expression every dispatch/counting site in this package embeds.
// For each combination of stored standing × benched_until state it writes a volunteer row,
// computes the Go verdict, then asks the DB for the verdict the shared SQL expression yields
// for the same row and asserts they are byte-identical. A change to EITHER side that drifts
// from the other fails here, so the SQL dispatch/counting standing can never silently
// diverge from the validation-side rule the two must agree on.
//
// The benched_until "past"/"future" cases sit a full hour from now so the small skew
// between the Go clock (EffectiveStanding's now argument) and the DB clock (standingExprSQL's
// NOW()) can never flip a verdict — the only place the two evaluate a live vs. expired bench.
func TestStandingExprSQL_MatchesEffectiveStanding(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := pool.QueryRow(ctx,
				`SELECT `+standingExprSQL("v")+` FROM volunteers v WHERE v.id = $1`, volID).
				Scan(&sqlStanding); err != nil {
				t.Fatalf("compute SQL standing: %v", err)
			}

			if goStanding != sqlStanding {
				t.Fatalf("standing twin drift (%s): volunteer.EffectiveStanding=%q, standingExprSQL=%q\n"+
