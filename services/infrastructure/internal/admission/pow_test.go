...
// manualDigest recomputes the solution digest independently of the admission package,
// spelling out the byte layout documented on VerifySolution:
//
...
// agree but the real admission.Solve / admission.VerifySolution assertions would break.
...
// a redeemed solution is single-purpose (see RedeemChallenge). The values are hardcoded
// from the reference solver so the negative case is deterministic — B's digest here has
// zero leading zero bits, well under the difficulty, so there is no probabilistic flake.
